...
class SettingsFragmentPresenter(private val fragmentView: SettingsFragmentView) {
    ...
    fun loadSettingsList() {
        ...
        when (menuTag) {
            ...
            Settings.SECTION_VR -> addVRSettings(sl)
            else -> {
    ...
    private fun addVRSettings(sl: ArrayList<SettingsItem>) {
        ...
        sl.apply {
            add(
                SingleChoiceSetting(
                    IntSetting.VR_ENVIRONMENT,
                    R.string.vr_background,
                    0,
                    R.array.vrBackgroundNames,
                    R.array.vrBackgroundValues,
                    IntSetting.VR_ENVIRONMENT.key,
                    IntSetting.VR_ENVIRONMENT.defaultValue
                )
            )
            add(
                SwitchSetting(
                    BooleanSetting.VR_EXTRA_PERFORMANCE_MODE,
                    R.string.vr_extra_performance_mode,
                    R.string.vr_extra_performance_mode_description,
                    BooleanSetting.VR_EXTRA_PERFORMANCE_MODE.key,
                    BooleanSetting.VR_EXTRA_PERFORMANCE_MODE.defaultValue
                )
            )
            add(
                SingleChoiceSetting(
                    IntSetting.VR_CPU_LEVEL,
                    R.string.vr_cpu_level,
                    R.string.vr_cpu_level_description,
